...
export function OpenSourceArticle({ entry, views, featured = false }: Props) {
  return (
    ...
    >
      <article className={`p-6 md:p-8 h-full flex flex-col ${featured ? 'md:p-10' : ''}`}>
        {/* Meta info */}
        ...
        {/* Title */}
        <h2 className={`font-semibold text-zinc-100 group-hover:text-white font-display transition-colors duration-300 flex items-start justify-between gap-2 ${
          ...
          <ArrowUpRight className={`flex-shrink-0 opacity-0 -translate-y-1 translate-x-1 group-hover:opacity-100 group-hover:translate-y-0 group-hover:translate-x-0 transition-all duration-300 text-zinc-400 ${
            featured ? 'w-6 h-6' : 'w-5 h-5'
          }`} />
        </h2>
        
        {/* Description */}
        <p className={`mt-3 text-zinc-400 group-hover:text-zinc-300 transition-colors duration-300 flex-grow ${
