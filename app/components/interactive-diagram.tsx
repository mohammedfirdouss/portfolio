...
export function InteractiveDiagram({ src, alt, title, description }: InteractiveDiagramProps) {
    ...
    <>
      ...
      >
        <div className="relative rounded-xl overflow-hidden border border-zinc-800 bg-zinc-900/50">
          ...
          {/* Interactive hints */}
          <div className={`absolute bottom-4 left-4 right-4 flex items-center justify-between transition-all duration-300 transform ${isHovered ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
        ...
        {(title || description) && (
          <div className="mt-3 flex flex-col gap-1 px-1">
            ...
            {description && <span className="text-xs text-zinc-500 italic">{description}</span>}
          </div>
        )}
      </div>
          ...
          <div className="relative w-full max-w-7xl max-h-screen overflow-auto flex flex-col items-center">
            ...
            >
              ...
              <div className="mt-6 max-w-2xl mx-auto text-center">
                <h3 className="text-2xl font-bold text-white mb-2">{title || alt}</h3>
                <p className="text-zinc-400">{description || "Interactive architecture diagram view."}</p>
              </div>
            </motion.div>
          </div>
        </motion.div>
