...
export default async function DiagramsPage() {
  const featured = allDiagrams.find((diagram) => diagram.slug === "terra-towns-diagram")!;
  const top2 = allDiagrams.find((diagram) => diagram.slug === "3-tier-architecture-using-aws-cloudformation")!;
      ...
      <div className="relative z-10 px-6 pt-24 mx-auto space-y-8 max-w-7xl lg:px-8 md:space-y-16 md:pt-32 lg:pt-40">
        ...
        <div className="max-w-2xl mx-auto lg:mx-0">
          ...
          <p className="mt-6 text-lg text-zinc-400 leading-relaxed">
            Visual diagrams and technical illustrations. Architecture designs, system flows, and infrastructure documentation.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 mx-auto lg:grid-cols-2">
          <Card>
            <Link href={`/diagrams/${featured.slug}`} data-cursor="pointer" data-cursor-text="View">
          ...
          <div className="flex flex-col w-full gap-6 mx-auto lg:mx-0">
            {[top2, top3].map((diagram) => (
              <Card key={diagram.slug}>
                <DiagramArticle entry={diagram} views={views[diagram.slug] ?? 0} />
              </Card>
            ))}
          </div>
        </div>
